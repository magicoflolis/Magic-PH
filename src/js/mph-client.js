...
if (typeof mph === 'object' && mph.randomToken instanceof Function === false) {

  mph.randomToken = function () {
    const n = Math.random();
    return (
      String.fromCharCode(n * 25 + 97) +
      Math.floor((0.25 + n * 0.75) * Number.MAX_SmphE_INTEGER)
        .toString(36)
        .slice(-8)
    );
  };

  mph.sessionId = mph.randomToken();
  ...
  mph.hermes = {
    ...
    canDestroyPort: function () {
      return (
        this.pending.size === 0 &&
        (this.extensions.length === 0 || this.extensions.every((e) => e.canDestroyPort()))
      );
    },
    ...
    portPoller: function () {
      ...
      this.portTimer = mph.setTimeout(this.portPollerBound, this.portTimerDelay);
    ...
    createPort: function () {
      ...
      try {
        this.port = browser.runtime.connect({ name: mph.sessionId }) || null;
      } catch (ex) {
        this.port = null;
      }
    ...
    //   Don't use `self` to access `mph`.
    extend: function () {
      if (this.extended === undefined) {
        this.extended = mph.hermes
          .send('mph', {
            what: 'extendClient'
          })
          .then(() => typeof mph === 'object' && this.extensions.length !== 0)
          .catch(() => {});
      }
