...
mph.Tabs = class {
  constructor() {
    webext.webNavigation.onCreatedNavigationTarget.addListener((details) => {
  ...
  replace(tabId, url) {
    ...
    mph.tabs.update(tabId, { url: targetURL });
  }
  ...
  async select(tabId) {
    ...
    const tab = await mph.tabs.update(tabId, { active: true });
    if (tab === null) {
      return;
    }
    if (mph.windows instanceof Object === false) {
  ...
  onFocusChangedHandler(windowId) {
    if (windowId === webext.windows.WINDOW_ID_NONE) {
      return;
    }
    mph.tabs.query({ active: true, windowId }).then((tabs) => {
      if (tabs.length === 0) {
        return;
      }
      const tab = tabs[0];
